...
func main() {
	...
	// Step 1:
	// generate version
	fmt.Println("Package Step 1: Generate Version")
	util.ExecCmd("python3 /stakater/pipeline-library/versioning/generate-version.py -f " + ciInfoFilePath + " -d " + *repoDirPathPtr)
	var appCiInfo infoUtil.AppCiInfo = infoUtil.ReadAppCiInfo(*repoDirPathPtr, *ciMetadataDirPathPtr)
	//TODO: Instead of printing, use in packaging
	fmt.Println("New Version:", appCiInfo.CiData.CurrentVersion)

	// Step 2:
	//run docker compose service
	var service = "package"
	fmt.Println("Package Step 2: Run docker-compose service:", service)
